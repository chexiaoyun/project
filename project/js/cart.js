$(function(){
	...
	//我的购物车，数量加
    $(".yd-goods-num .yd-num-right").click(function(){
        var num = parseInt( $(this).prev('.yd-num-input').find("input").val() );   //找到input
        num++;
        //var parice = $(this).parents('ul').find(".price span").html();             //物品的单价
        //var subtotal = parseFloat(parice)*num;
        //$(this).parents('ul').find(".subtotal span").html(subtotal);               //总价格
        $(this).prev('.yd-num-input').find("input").val(num);
        //total();
    });

    //我的购物车，数量减
    $(".yd-goods-num .yd-num-left").click(function(){
        var num = parseInt( $(this).next('.yd-num-input').find("input").val() );
        if (num > 1) {
            num--;
        }
        ...
        //$(this).parents('ul').find(".subtotal span").html(subtotal);
       	$(this).next('.yd-num-input').find("input").val(num);
       	//total();
    });
